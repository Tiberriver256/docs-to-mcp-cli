...
- POST /api/update`,
    ...
    const cliCmd = `node dist/src/index.js --docs "${join(docsDir, '*.md')}" --packageName ${packageName} --outDir ${outputDir}`;
    
    const { stdout } = await execAsync(cliCmd);
    console.log(stdout);
    
    // Verify server files were created
    assert.ok(existsSync(join(outputDir, 'index.js')), 'index.js should exist');
    assert.ok(existsSync(join(outputDir, 'package.json')), 'package.json should exist');
  ...
  }, { timeout: 60000 }); // Increase timeout for dependency installation
  ...
  test('should connect to server and call list_docs', async () => {
    ...
    try {
      ...
      assert(content[0].text.length > 0, "First content item text length should be greater than 0");
    } catch (error) {
  ...
  test('should get a specific doc with path relative to test dir', async () => {
    ...
    try {
      ...
      const content = getDocResult.content as Array<{ type: string; text: string }>;
      assert(content.length > 0, "Content length should be greater than 0 for get_doc");
    } catch (error) {
  ...
  test('should search docs', async () => {
    try {
      ...
      assert(content.length > 0, "Content length should be greater than 0 for search_docs");
    } catch (error) {
